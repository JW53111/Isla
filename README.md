...
这个仓库内置了一只生成好的桌宠 **isla**（艾拉：银白长发、红瞳、白色制服 + 红色胸结的 Q 版形象），共 **12 个动作**：
...
| sulk | 生闷气 | 戳她时 35% 几率背过身去 / 3 分钟不互动自己生闷气，5 秒后自己回来 |
| celebrate | 完成任务庆祝 | 右键切换 |
| sleep | 睡觉挂机 | 双击她切换 / 10 分钟不互动自动睡 |
| new-message | 消息提醒 | 右键切换 |
| date-weather | 日期天气播报 | 右键切换（联网拉取实时日期/城市/天气/温度，头顶气泡显示） |

### 怎么召唤 isla（Windows）

**双击项目根目录的 `启动isla桌宠.bat`**，她就会出现。第一次双击后会自动在桌面创建带 isla 图标的快捷方式 **Isla**，以后双击桌面图标即可召唤。
...
| 操作 | 效果 |
|---|---|
| **右键点击她** | 弹出动作菜单：切换任意动作 / 缩放 / 退出 |
| **按住拖动** | 移动她的位置（位置会记住，重启后回到原处） |
| **单击她（身体）** | 按顺序循环切换动作（敲键盘/睡觉/撒娇等专属动作不在循环内） |
| **双击她（身体）** | 睡觉 ↔ 唤醒 |
| **单击她（头部）** | 摸头：戳一戳撒娇（35% 几率背过身生闷气）；6 秒内连摸 3 次 → 傲娇打气 |
| 放着不动 | 10 分钟不互动自动睡觉；3 分钟不互动自己生闷气；偶尔在屏幕底部小范围漂移 |
| 你敲键盘 | 她切换到「敲键盘」动作，跟你哒哒打字，停手 1.5 秒回到待机 |
| 你在别处点鼠标 | 她安静回到「陪伴待机」（不再弹撒娇动作；你右键/单击循环选的动作不受影响） |
| 你移动鼠标 | 她的红瞳跟着你的鼠标转，一直盯着光标看（眨眼/闭眼时自然不跟） |
| 切到「日期天气」 | 头顶弹出气泡：日期+星期 / 城市 天气 温度（联网实时获取，失败时只显示日期） |
...
托盘区有 isla 图标但看不到她，通常是**默认动作 id 不匹配**导致画面一直空白。当前 `renderer/app.js` 已修复：默认动作会优先取 `idle`，没有则取 `idle-buddy`，再没有则取动作表的第一个动作。如果你的桌宠动作表没有叫 `idle` 的动作，确认用的是修复后的 `app.js`。
...
导出模板里已经内置 `npmRebuild: false`（不需要 Visual Studio）和 `signAndEditExecutable: false`（跳过 winCodeSign 下载，代价是 exe 文件图标为默认 Electron 图标，不影响功能）。图标通过**桌面快捷方式**解决：启动 bat 会自动创建/刷新带宠物图标的桌面快捷方式，托盘图标也正常。

这个模块加载失败时应用也会**优雅降级**：键盘镜像不可用 → 桌宠不会跟着打字，但桌宠本身不会崩溃。
