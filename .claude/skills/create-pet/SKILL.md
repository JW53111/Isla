...
| `sulk` | 生闷气：被戳时随机 35% 触发 + 3 分钟不互动触发，5 秒后自己回来 | 被戳只播 poke-react |
| `date-weather` | 日期天气播报：主进程联网拉实时天气（免 key 接口），头顶气泡显示日期/城市/天气/温度，10 分钟缓存，失败只显示日期 | 无播报动作 |

`SEMANTIC_ACTIONS = ['idle', 'typing', 'sleep', 'poke-react', 'sulk']` 由导出脚本生成：语义动作不参与**单击循环切换**（它们各有专属触发方式）。桌宠不会自己随机切动作；只有 10 分钟不互动自动睡觉、3 分钟不互动生闷气是自主行为。

**眼神跟随鼠标（运行时自动）**：`sprite-engine.js` 加载精灵图时会扫描每帧头部区域（帧高 18%~37%）的**偏红瞳色像素**（`r > 100 && r > g*1.6 && r > b*1.6`），连通域配对定位双眼，渲染时在眼珠上叠画瞳孔并随鼠标偏移。闭眼/侧脸/背身帧检测不到就自动不画（天然兼容眨眼和睡觉）。**只对偏红瞳色有效**——设计动作 prompt 时如果角色不是红瞳、又想要眼神跟随，可以要求精灵图把瞳色画成偏红（如「红宝石色瞳孔」），否则该功能静默降级（眼睛不动，无任何报错）。瞳孔/眨眼线用深灰黑色（如 `rgba(22,16,18,…)`），偏红会显诡异。

**帧切换一律硬切（重要经验）**：动作内帧切换、动作间切换都直接换帧，**不要实现任何交叉淡入/叠旧帧过渡**——实测叠旧帧的淡入会产生重影闪烁，用户实感就是「闪」；硬切虽然略僵硬但观感干净，用户明确接受「僵硬但不闪」。眨眼、呼吸、重心稳定、眼神跟随、头部微动等运行时动态可以保留（它们不叠帧、不闪）。
...
- `assets/icon.png`：从清理后的 reference 图等比居中生成 1024x1024 透明 PNG（托盘图标用）。
- `assets/icon.icns`：mac 打包用图标，优先由 `iconutil` 从 PNG iconset 生成。
- `assets/icon.ico`：Windows 用，从 icon.png 内嵌多尺寸 PNG（16/32/48/64/128/256）生成。
- 如果本机无法生成 `.icns`，才回退到项目默认图标。
- 不要继续使用通用默认图标作为最终安装包图标。
- 默认图标背景使用透明背景，不要用纯绿背景；除非用户明确要求白底，才改成白色背景。

**Windows exe 文件图标写不进去（已验证两条路都不可行，别再试）**：①`signAndEditExecutable: true` 时 winCodeSign 解压在无符号链接权限的 Windows 上必失败（darwin .dylib 软链）；②手动 rcedit（旧版）处理不了 Electron 大 exe，会把 176MB 文件写坏成 3.4GB。**替代方案**：桌面快捷方式 `.lnk` 用 `assets/icon.ico` 作图标（WScript.Shell COM 创建，`IconLocation` 指向 ico），启动 bat 每次启动静默创建/刷新；托盘图标用 `icon.png`，不受影响。
