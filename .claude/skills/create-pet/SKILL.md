...
**眼神跟随鼠标（运行时自动）**：`sprite-engine.js` 加载精灵图时会扫描每帧头部区域（帧高 18%~37%）的**偏红瞳色像素**（`r > 100 && r > g*1.6 && r > b*1.6`），连通域配对定位双眼，渲染时在眼珠上叠画瞳孔并随鼠标偏移。闭眼/侧脸/背身帧检测不到就自动不画（天然兼容眨眼和睡觉）。**只对偏红瞳色有效**——设计动作 prompt 时如果角色不是红瞳、又想要眼神跟随，可以要求精灵图把瞳色画成偏红（如「红宝石色瞳孔」），否则该功能静默降级（眼睛不动，无任何报错）。
